...
create_lease(Identifier, LeaseDuration, Range, Table) ->
	...
	Lease = #dhcp_lease{
	   ...
	   state = offered,
	   expiration = erlang:send_after(?DEFAULT_OFFER_TIMEOUT_SECONDS*1000, self(), {expire, Identifier}),
...
next_ip(Range, Table) -> 
    ...
    Bcast = ip_to_int(End),
    Used = lists:sort([ip_to_int(IP) || #dhcp_lease{ip = IP} <- Table,
                       inet_cidr:contains({Start, End, Prefix}, IP)]),
    scan(Used, Low, Bcast).

ip_to_int({A,B,C,D}) -> <<N:32>> = <<A:8,B:8,C:8,D:8>>, N.
int_to_ip(N)         -> <<A:8,B:8,C:8,D:8>> = <<N:32>>, {A,B,C,D}.
